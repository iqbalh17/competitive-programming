...
void Main(int tc) {
	...
	for(int i = 0; i < n; i++) {
		cin >> v[i].se >> v[i].fi.fi;
		v[i].fi.se = 0;
		if(v[i].fi.fi != 0){
			v[i].fi.se = v[i].fi.fi - 24;	
		}	
	}
	sort(all(v));
	int ans = -1;
	for(int i = 0; i < n; i++){
		for(int k = 0; k < 9; k++){
			...
			for(int j = 0; j < n; j++){
				if(j == i) continue;
				int id = min(abs(v[j].fi.fi-v[i].fi.fi), abs(v[j].fi.se-v[i].fi.fi));
				if(id == abs(v[j].fi.se-v[i].fi.fi)) id *= -1;
				if(id == abs(v[j].fi.fi-v[i].fi.fi) && v[j].fi.fi < v[i].fi.fi) id*=-1;
					if(k + id >=0 && k + id <= 8){
						sum += v[j].se;
					}
			}
